...
type t = nat [@@deriving show { with_path = false }]
...
let turn_kind ~current_level ~level =
  ...
  if remainder = Z.zero then Committer
  else if remainder = Z.one then Fork_committer
  else if remainder = Z.of_int 2 then Rejector
  else Fork_rejector
